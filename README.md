...
- [ ] **Entities**:
    - [ ] Map `Address` entity.
    - [ ] Map `UserProfile` entity.
    - [ ] Map `Customer` entity (including relationships).
- [ ] **Repositories**:
    - [ ] Create `AddressRepository` with required query methods.
    - [ ] Create `UserProfileRepository` with required query methods.
    - [ ] Create `CustomerRepository` with required query methods.
...
Create a new branch for Part 3
Review Dependencies: Ensure spring-boot-starter-validation is in pom.xml for DTO validation.
Optional: Add MapStruct dependency if you want to explore automated mapping (though manual mappers are recommended for learning).
...
Create a package se.lexicon.ecommerceworkshop.dto. In this part, we use Java Records for our DTOs. Records are immutable, provide a concise syntax, and are perfect for data transfer.
...
3. OrderService (Business Process)
   ...
   Business Logic:
   Find the Customer.
   For each requested item, find the Product.
...
2. PromotionService (Marketing Logic)
   ...
   Example:
   The sequence diagram shows the complete process of placing an order in the system. It explains how the OrderService manages the workflow by communicating with Mappers, Repositories, and the Database, while ensuring everything happens inside a single transaction.
